...
class NET(nn.Module):
    ...
    def forward(self, x):
        ...
        convpool5 = nn.MaxPool2d(kernel_size = 5, stride=1,padding=2, return_indices=False)
    ...
    def loadimage(self,path):
        ...
        H,W = (H//self.modelpooling)*self.modelpooling,(W//self.modelpooling)*self.modelpooling
    ...
    def forwardnumpy(self, x):
        inputtensor = torch.autograd.Variable(torch.Tensor(np.expand_dims(x,axis=0)).cuda(), requires_grad=False)
        with torch.no_grad():
    ...
    def computemetric(self,prob,centers):
        ...
        nbGoodmatch,goodalarme,catcheddetection,match = computeprecisionrappel(prediction,centers,4)
        ...
        return (pred, np.asarray([nbposit, nbpropo, nbGoodmatch ,nbclearfalsealarm]), coloredimage)
    ...
    def stdtest(self, foldertest,folderout):
        ...
        for name in imagename:
            ...
            coloredimagescaled = np.zeros((coloredimage.shape[0]*self.modelpooling,coloredimage.shape[1]*self.modelpooling,3),dtype=int)
            for r in range(coloredimage.shape[0]):
                for c in range(coloredimage.shape[1]):
                    for i in range(3):
                        coloredimagescaled[r*self.modelpooling:(r+1)*self.modelpooling,c*self.modelpooling:(c+1)*self.modelpooling,i:i+1]=coloredimage[r][c][i]
        ...
        if nbpropo==0:
            nbpropo=1
        return (nbposit, nbpropo, nbcorrect, 100*nbcorrect/nbposit,100*nbcorrect/nbpropo,100*nbcorrect/nbpropo*nbcorrect/nbposit,nbclearfalsealarm)
