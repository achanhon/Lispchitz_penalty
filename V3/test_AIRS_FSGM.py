...
def iou(cm):
    return 0.5 * (
        cm[0][0] / (cm[0][0] + cm[0][1] + cm[1][0] + 1)
        + cm[1][1] / (cm[1][1] + cm[0][1] + cm[1][0] + 1)
    )
...
for name in datatest.names:
    print(name)
    im = (
        PIL.Image.open(datatest.root + "/" + datatest.images[name])
        .convert("RGB")
        .copy()
    )
    im = np.asarray(im, dtype=np.uint8)  # warning wh swapping
    imagetensor = (
        torch.Tensor(np.transpose(im, axes=(2, 0, 1))).cpu().to(device).unsqueeze(0)
    )
    size = (imagetensor.shape[2] // 64) * 64, (imagetensor.shape[3] // 64) * 64
    x = F.interpolate(imagetensor, size)

    realmask = (
        PIL.Image.open(datatest.root + "/" + datatest.masks[name]).convert("RGB").copy()
    )
    realmask = realmask.resize(
        ((imagetensor.shape[3] // 64) * 64, (imagetensor.shape[2] // 64) * 64),
        PIL.Image.NEAREST,
    )
    ...
    localcm = confusion_matrix(
        z.cpu().numpy().flatten(), y.cpu().numpy().flatten(), [0, 1]
    )
    # print(localcm)
    ...
    for a, b, c, d in [
        (0, x.shape[2] // 2, 0, x.shape[3] // 2),
        (x.shape[2] // 2, x.shape[2], 0, x.shape[3] // 2),
        (0, x.shape[2] // 2, x.shape[3] // 2, x.shape[3]),
        (x.shape[2] // 2, x.shape[2], x.shape[3] // 2, x.shape[3]),
    ]:
        ...
        localcmfsgm = confusion_matrix(
            z.cpu().numpy().flatten(), y[:, a:b, c:d].cpu().numpy().flatten(), [0, 1]
        )
        # print(localcmfsgm)
