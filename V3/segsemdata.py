...
class SegSemDataset:
    ...
    def getrawrandomtiles(self, nbtilesperimage, h, w, batchsize):
        ...
        for name in self.names:
            col = np.random.randint(
                0, self.sizes[name][0] - w - 2, size=nbtilesperimage
            )
            row = np.random.randint(
                0, self.sizes[name][1] - h - 2, size=nbtilesperimage
            )

            image = (
                PIL.Image.open(self.root + "/" + self.images[name])
                .convert("RGB")
                .copy()
            )
            ...
            label = (
                PIL.Image.open(self.root + "/" + self.masks[name]).convert("RGB").copy()
            )
    ...
    def apply(self, model, tilesize, stride, pathout=""):
        ...
        with torch.no_grad():
            for imageindex, name in enumerate(self.names):
                ...
                realmask = (
                    PIL.Image.open(self.root + "/" + self.masks[name])
                    .convert("RGB")
                    .copy()
                )
                ...
                imagetensor = (
                    torch.Tensor(np.transpose(im, axes=(2, 0, 1))).cpu().unsqueeze(0)
                )
                ...
                if (
                    imagetensor.shape[2] <= 3 * tilesize
                    and imagetensor.shape[2] <= 3 * tilesize
                ):
                    # only for small image
                    size = (imagetensor.shape[2] // 32) * 32, (
                        imagetensor.shape[3] // 32
                    ) * 32
                    imagetensor = F.interpolate(imagetensor, size)
                    mask = model(imagetensor).cpu()
                else:
                    # resize and cut in tile
                    size = (imagetensor.shape[2] // tilesize) * tilesize, (
                        imagetensor.shape[3] // tilesize
                    ) * tilesize
                    imagetensor = F.interpolate(imagetensor, size)

                    mask = torch.zeros(
                        1,
                        len(self.setofcolors),
                        imagetensor.shape[2],
                        imagetensor.shape[3],
                    ).cpu()
                    for row in range(0, size[0] - tilesize + 1, stride):
                        for col in range(0, size[1] - tilesize + 1, stride):
                            mask[
                                :, :, row : row + tilesize, col : col + tilesize
                            ] += model(
                                imagetensor[
                                    :, :, row : row + tilesize, col : col + tilesize
                                ]
                ...
                if pathout != "":
                    ...
                    if imageindex % 10 == 0:
                        ...
                        image = PIL.Image.fromarray(
                            self.vtTOcolorvt(self.colorvtTOvt(realmask))
                        )
    ...
    def vtTOcolorvt(self, mask):
        ...
        for i in range(len(self.setofcolors)):
            for ch in range(3):
                maskcolor[:, :, ch] += ((mask == i).astype(int)) * self.setofcolors[i][
                    ch
                ]
    ...
    def colorvtTOvt(self, maskcolor):
        ...
        for i in range(len(self.setofcolors)):
            mask += (
                i
                * (maskcolor[:, :, 0] == self.setofcolors[i][0]).astype(int)
                * (maskcolor[:, :, 1] == self.setofcolors[i][1]).astype(int)
                * (maskcolor[:, :, 2] == self.setofcolors[i][2]).astype(int)
            )
        return mask
...
class MergedSegSemDataset:
    ...
    def getrandomtiles(self, nbtilesperimage, h, w, batchsize):
        ...
        for dataset in self.alldatasets:
            XY = dataset.getrawrandomtiles(nbtilesperimage, h, w, batchsize)
            X = X + [torch.Tensor(np.transpose(x, axes=(2, 0, 1))).cpu() for x, y in XY]
            Y = Y + [
                torch.from_numpy(dataset.colorvtTOvt(y)).long().cpu() for x, y in XY
            ]
...
def resizeDataset(
    ...
    for name, (x, y) in XY.items():
        ...
        image = image.resize(
            (
                int(image.size[0] * nativeresolution / resolution),
                int(image.size[1] * nativeresolution / resolution),
            ),
            PIL.Image.BILINEAR,
        )
        if not color:
            image = torchvision.transforms.functional.to_grayscale(
                image, num_output_channels=1
            )
        if normalize:
        ...
        maskc = maskc.resize(
            (
                int(maskc.size[0] * nativeresolution / resolution),
                int(maskc.size[1] * nativeresolution / resolution),
            ),
            PIL.Image.NEAREST,
        )
...
class Unet(nn.Module):
    ...
    def pool2GT(self, targets):
        resizedtarget = torch.zeros(
            targets.shape[0], targets.shape[1] // 2, targets.shape[2] // 2
        )
        for i in range(targets.shape[0]):
            maskc = PIL.Image.fromarray(np.uint8(targets[i].cpu().numpy()))
            maskc = maskc.resize(
                (maskc.size[0] // 2, maskc.size[1] // 2), PIL.Image.NEAREST
            )
    ...
    def computeloss(self, inputs, targets, criterion):
        ...
        losses = [
            criterion(allprob[i], alltarget[i].long()) * pow(0.8, i) for i in range(5)
        ]
        loss = sum(losses)
        return loss, prob
...
def makeDFC2015(
    resolution=50, datasetpath="/data/DFC2015", mode="lod0", color=False, normalize=True
):
    XY = {}
    ...
    return resizeDataset(
        XY, datasetpath, DFC2015color(mode), 5, resolution, color, normalize
    )


def maketrainDFC2015(
    resolution=50,
    datasetpath="/data/DFC2015",
    mode="normal",
    color=True,
    normalize=False,
):
...
def makeISPRS_VAIHINGEN(
    resolution=50,
    datasetpath="/data/ISPRS_VAIHINGEN",
    mode="lod0",
    color=False,
    normalize=True,
):
    ...
    return resizeDataset(
        XY, datasetpath, ISPRScolor(mode), 9, resolution, color, normalize
    )


def makeISPRStrainVAIHINGEN(
    resolution=50,
    datasetpath="/data/ISPRS_VAIHINGEN",
    mode="lod0",
    color=False,
...
def makeISPRS_POSTDAM(
    resolution=50, datasetpath="/data/POSTDAM", mode="lod0", color=False, normalize=True
):
    names = [
        "top_potsdam_2_10_",
        "top_potsdam_2_11_",
    ...
    return resizeDataset(
        XY, datasetpath, ISPRScolor(mode), 5, resolution, color, normalize
    )
...
def makeAIRSdataset(datasetpath, resolution=50, color=False, normalize=True, size=0):
    ...
    return resizeDataset(
        XY, datasetpath, [[0, 0, 0], [255, 255, 255]], 7.5, resolution, color, normalize
    )


def makeTrainAIRSdataset(
    datasetpath="/data/AIRS/train", resolution=50, color=False, normalize=True, size=33
):
    allfile = os.listdir(datasetpath + "/image")
    if size > 0:
        random.shuffle(allfile)
...
def makeTestAIRSdataset(
    datasetpath="/data/AIRS/val", resolution=50, color=False, normalize=True, size=33
):
    allfile = os.listdir(datasetpath + "/image")
    if size > 0:
        random.shuffle(allfile)
