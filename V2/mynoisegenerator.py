...
def implementedperturbation():
    return ["FSGM","randomsign","gaussian","pepperandsalt","uniform","blur","center","identity"]

def ensureimage(x,device="cuda"):
    return torch.min(255*torch.ones(x.shape).to(device),torch.max(torch.round(x),torch.zeros(x.shape).to(device)))
...
def perturbinput(x,y,f,level,mode,nbclasses):
    ...
    if mode=="randomsign":
        ...
        with torch.no_grad():        
            for i in range(500):
                ...
                ze = f(xe)
                _,pred = ze.max(1)
                cm = confusion_matrix(pred.cpu().detach().numpy().flatten(),y.cpu().numpy().flatten(),list(range(nbclasses)))
    ...
    if mode=="FSGM":    
        ...
        for i in range(level):
            ...
            loss = criterion(torch.flatten(f(xe),start_dim=2), torch.flatten(y,start_dim=1))
            optimizer.zero_grad()
            loss.backward()
    ...
    xe,ze,pred,y,d1,d2 = xe.cpu().detach().numpy(),ze.cpu().detach().numpy(),pred.cpu().numpy(),y.cpu().numpy(),d1.cpu().detach().numpy(),d2.cpu().detach().numpy()
...
def worseperturbation(x,y,f,level,nbclasses):
    bestaccuracy = 2
    best = None
    for mode in ["FSGM","randomsign","gaussian","pepperandsalt","uniform","blur","center"]:
        xe,ze,pred,cm,accuracy, d1,d2 = perturbinput(x,y,f,level,mode,nbclasses)
        if accuracy<bestaccuracy:
