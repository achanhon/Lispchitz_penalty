...
class Unet(nn.Module):
    ...
    def computeloss(self, inputs, targets, criterion):
        ...
        losses = [
            criterion(
                torch.flatten(allprob[i], start_dim=2),
                torch.flatten(alltarget[i], start_dim=1).long(),
            )
            * pow(0.8, i)
            for i in range(5)
        ]
...
def makeISPRStrainVAIHINGEN(
    resolution=50, datasetpath="", mode="lod0", color=False, normalize=True
):
    names = [
...
def makeINRIAdataset(mode, resolution=50, color=False, normalize=True):
    ...
    if mode not in ["Train", "Test"]:
        allfile = allfile
    return resizeDataset(
        XY, datasetpath, [[0, 0, 0], [255, 255, 255]], 9, resolution, color, normalize
    )
