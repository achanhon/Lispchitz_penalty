    ...
    def getbatchloader(self, nbtiles=10000, tilesize=256, batchsize=32, nbpos=1):
        ...
        for name in self.names:
            ...
            for i in range(NB):
                ...
                mask = label[
                    row[i] : row[i] + tilesize // 16, col[i] : col[i] + tilesize // 16
                ]
            ...
            if nbpos != 0 and np.sum(label) > 1:
                ...
                noise = np.random.randint(
                    -tilesize // 16, tilesize // 16, size=(len(l), 2)
                )
...
if __name__ == "__main__":
    ...
    globalresize = torch.nn.AdaptiveAvgPool2d((debug.shape[0], debug.shape[1]))
