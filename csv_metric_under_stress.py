...
for intensity in range(10):
    print("@@@@@@@@@@@@@@@@@ stress = adversarial with intensity ",intensity+1, " ===>")
    metric = np.zeros(4,dtype=float)
    ...
    for i in range(len(X)):
        ...
        variableimage = torch.autograd.Variable(torch.from_numpy(np.expand_dims(x,axis=0)).cuda().float(),requires_grad=True)
...
with torch.no_grad():
    ...
    print("#####################################################################")
    allstress = [("hard_sparse_noise",list(range(1,15))), ("illumination_change",list(range(1,15))),("gaussian_noise",list(range(1,15)))]
        ...
        for intensity in possibleintensity:
            ...
            for name in imagename:
                ...
                if stress == "hard_sparse_noise":
                    nbRand = image.shape[0]*image.shape[1]*image.shape[2]*intensity//100+1
                    randindex = np.stack([np.random.randint(0,image.shape[0],size=nbRand),np.random.randint(0,image.shape[1],size=nbRand),np.random.randint(0,image.shape[2],size=nbRand)],axis=0)
                    for i in range(randindex[0].shape[0]):
                        image[randindex[0][i]][randindex[1][i]][randindex[2][i]] = 255*random.randint(0,1)
            ...
            print("@@@@@@@@@@@@@@@@@ stress = ", stress," with intensity ",intensity, " ===>")
            print("recall=",100*nbcorrect/nbposit)
            print("precision=",100*nbcorrect/nbpropo)
            print("gscore=",100*nbcorrect/nbpropo*nbcorrect/nbposit)
            print("nb hard false alarms=",nbclearfalsealarm)
